...
- A Hugging Face access token with read access to the backend Space if that
  Space is private. This is stored locally as `HF_TOKEN`.
...
## 7. Configure both authentication layers
...
If the backend Space is private, create a Hugging Face access token belonging
to a user or service account that can access that Space. A fine-grained token
limited to read access for the Space is preferred; a general read token also
works when its owner has access to the organization Space.

These tokens do different jobs:

- `HF_TOKEN` lets the request enter the private Hugging Face Space.
- `WORKER_TOKEN` is checked by the FastAPI worker endpoints after the request
...
- **404 Not Found for `/train_jobs/next`**: the worker reached a backend, but
  either the private Space rejected the request before it reached FastAPI or
  the live deployment lacks the training endpoints. Set `HF_TOKEN` to a token
  whose owner can access the private Space. Also make sure `BACKEND_URL` is the
  backend Space and that its deployment includes `/train_jobs/next`,
  `/train_jobs/{job_id}/bundle`, `/heartbeat`, and `/artifacts`.
- **Ultralytics reports that `images/train` is missing**: update to the latest
  worker files and restart it. The worker now rewrites the bundle's `path: .`
  entry to the absolute Windows extraction path in `data.worker.yaml`, avoiding
  Ultralytics' global `datasets_dir` resolution. Submit a new fine-tune job
  after updating because the failed job is already terminal.
