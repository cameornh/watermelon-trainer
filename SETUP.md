...
- **404 Not Found for `/train_jobs/next`**: the worker reached a backend, but
  that live Space does not have the training-worker endpoints deployed. Make
  sure `BACKEND_URL` is the backend Space, not the frontend or proxy, and push /
  rebuild the backend version that includes `/train_jobs/next`,
