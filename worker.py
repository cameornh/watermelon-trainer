...
if HF_TOKEN:
    # Required for private Hugging Face Spaces. Hugging Face authenticates this
    # request before it reaches the FastAPI app and its X-Worker-Token check.
...
def describe_claim_error(exc: Exception) -> str:
    ...
    if response is not None:
        ...
        if status == 404:
            ...
            auth_hint = (
                " HF_TOKEN is not configured, so a private Hugging Face Space "
                "will intentionally appear as 404."
                if not HF_TOKEN
                else " HF_TOKEN was supplied; confirm that it belongs to a user or "
                "service account with read access to this private Space."
            )
            return (
                ...
                f"training endpoints were not pushed/rebuilt there yet.{auth_hint}"
            )
        if status == 401:
            return (
...
def _prepare_data_yaml(work: Path) -> Path:
    """Rewrite the bundle YAML with an absolute native dataset root.

    Ultralytics does not consistently resolve ``path: .`` relative to the
    YAML file on Windows. Depending on its global ``datasets_dir`` setting, it
    can instead look for ``images/train`` beside the trainer directory. The
    worker is the first place that knows the bundle's final extraction path,
    so make that root explicit.
    """
    import yaml

    source = work / "data.yaml"
    ...
    try:
        ...
        raise RuntimeError(f"could not read bundle data.yaml: {exc}") from exc
    if not isinstance(data, dict):
        raise RuntimeError("bundle data.yaml must contain a YAML mapping")

    train_rel = str(data.get("train") or "images/train")
    val_rel = str(data.get("val") or train_rel)
    train_dir = work / Path(train_rel)
    val_dir = work / Path(val_rel)

    if not train_dir.is_dir():
        raise FileNotFoundError(f"bundle training images directory is missing: {train_dir}")
    if not any(path.is_file() for path in train_dir.rglob("*")):
        raise FileNotFoundError(f"bundle training images directory is empty: {train_dir}")
    if not val_dir.is_dir():
        raise FileNotFoundError(f"bundle validation images directory is missing: {val_dir}")

    # Forward slashes are valid on Windows and avoid backslash escaping in
    # YAML. JSON is valid YAML and safely preserves drive letters and spaces.
    data["path"] = work.resolve().as_posix()
    data["train"] = train_rel.replace("\\", "/")
    data["val"] = val_rel.replace("\\", "/")
    prepared = work / "data.worker.yaml"
...
def _validate_config() -> bool:
    ...
    if not WORKER_TOKEN:
        ...
        ok = False
    if not HF_TOKEN and BACKEND_URL.endswith(".hf.space"):
        _log(
            "WARNING: HF_TOKEN is not set. This is fine for a public Space, but "
            "a private Hugging Face Space will return 404 before requests reach the worker API."
        )
    return ok
...
def main() -> int:
    ...
    _log(f"Hugging Face private-Space authentication: {'configured' if HF_TOKEN else 'not configured'}")
    _report_gpu()
